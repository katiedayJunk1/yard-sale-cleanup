...
function signupConfirmation({ firstName, minRequired, price, manageUrl, weekLabel }) {
  ...
  const text = [
    ...
    `Submitting this form reserves your spot, but your pickup is not confirmed yet.`,
    `If we reach ${minRequired} signups, you will receive an invoice/payment link for ${formatMoney(price)}.`,
    `Your pickup is confirmed once payment is completed.`,
    '',
...
function dealIsOn({ firstName, price, weekLabel, manageUrl }) {
  ...
  const text = [
    ...
    `Your Yard Sale Leftovers Pickup is eligible for the ${formatMoney(price)} deal.`,
    `You will receive an invoice/payment link. Your pickup is confirmed once payment is completed.`,
    '',
    `Manage/cancel: ${manageUrl}`,
    '',
